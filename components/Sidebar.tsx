...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-white z-30 relative w-full">
      {/* User Header */}
      <div className="bg-white px-6 md:px-8 py-6 md:py-8 flex items-center justify-between border-b border-gray-50">
        <div className="flex items-center gap-4 md:gap-5 group cursor-pointer" onClick={() => setModalType('invite')}>
          ...
          <div className="overflow-hidden">
            ...
            <div className="flex items-center gap-2 mt-1">
               ...
               <p className="text-[10px] text-teal-600 font-black uppercase tracking-[0.25em]">Local Node Active</p>
            </div>
          </div>
        </div>
      ...
      {/* Dynamic HUD */}
      <div className="px-6 md:px-8 py-3 bg-teal-50/40 flex items-center justify-between border-b border-teal-50">
         <div className="flex items-center gap-2.5">
            ...
            <span className="text-[11px] font-black text-teal-900 uppercase tracking-widest">Mesh Health</span>
         </div>
      ...
      <div className="px-6 md:px-8 py-4 md:py-6 bg-white">
        <div className="relative flex items-center bg-gray-50 rounded-2xl md:rounded-3xl px-4 md:px-6 py-3 md:py-4 border-2 border-transparent focus-within:border-teal-100 focus-within:bg-white focus-within:shadow-2xl focus-within:shadow-teal-600/5 transition-all">
          ...
          <input 
            type="text" 
            placeholder="Search the node list..." 
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-5 pb-8 custom-scrollbar space-y-2">
        {filteredChats.length > 0 ? (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                  ...
                  {chat.lastMessage && (
                    <span className={`text-[10px] font-black uppercase tracking-tighter mt-1 ${activeChatId === chat.id ? 'text-teal-100/70' : 'text-gray-300'}`}>
                ...
                <div className="flex items-center gap-2">
                  <p className={`text-[13px] md:text-[14px] truncate flex-1 font-medium transition-colors ${activeChatId === chat.id ? 'text-teal-50/80' : 'text-gray-500'}`}>
                    {chat.isTyping ? (
                      <span className="italic animate-pulse">Syncing neural input...</span>
                    ) : chat.lastMessage ? chat.lastMessage.content : 'P2P Handshake Ready'}
                  </p>
                  {chat.lastMessage?.senderId === user.id && activeChatId !== chat.id && (
                     <CheckCheck size={16} className={chat.lastMessage.status === 'read' ? 'text-teal-400' : 'text-gray-300'} />
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="p-8 md:p-12 text-center mt-10">
            ...
            <p className="text-gray-900 font-black text-xl md:text-2xl tracking-tighter mb-4">Isolated Node</p>
            <p className="text-sm text-gray-400 leading-relaxed font-bold px-4">Your communications are strictly local. Extend your reach by sharing your node identity link.</p>
            <button 
      ...
      {/* Modals */}
      {modalType !== 'none' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/70 backdrop-blur-xl p-4 md:p-8 animate-in fade-in duration-500">
          <div className="bg-white rounded-[2.5rem] md:rounded-[3.5rem] shadow-[0_50px_120px_-30px_rgba(0,0,0,0.4)] w-full max-w-sm overflow-hidden animate-in zoom-in duration-500 border border-white/30">
            <div className="p-8 md:p-12">
              ...
              {modalType === 'invite' && (
                <div className="space-y-8 md:space-y-10">
                  <div className="bg-teal-50/50 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-teal-100 flex flex-col items-center shadow-inner">
                    <div className="w-16 h-16 md:w-20 md:h-20 bg-white rounded-3xl flex items-center justify-center shadow-xl mb-4 md:mb-6 text-teal-600">
                       <LinkIcon size={32} />
                    </div>
                    <p className="text-[10px] font-black text-teal-800 uppercase tracking-[0.4em] mb-4 md:mb-6">Node Invite</p>
                    <code className="w-full block text-teal-700 font-mono text-[9px] md:text-[10px] break-all bg-white/80 p-4 md:p-6 rounded-xl border border-teal-100 leading-relaxed text-center shadow-sm">
                      {inviteLink}
                    </code>
                  </div>
                  <button 
                    ...
                    className="w-full bg-teal-600 text-white py-5 md:py-6 rounded-2xl md:rounded-3xl font-black text-lg md:text-xl hover:bg-teal-700 transition-all shadow-2xl shadow-teal-600/30 active:scale-95"
                  >
                    Copy Link
                  </button>
                </div>
              ...
              {modalType === 'connect' && (
                <div className="space-y-8 md:space-y-10">
                  <div className="space-y-3">
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.3em] px-3">Target Node Address</label>
                    <input 
                      autoFocus
                      type="text" 
                      placeholder="Node UID..."
                      className="w-full px-6 py-4 md:px-8 md:py-6 bg-gray-50 border-2 border-transparent focus:border-teal-500 focus:bg-white rounded-[1.25rem] md:rounded-[1.75rem] outline-none transition-all text-sm font-bold shadow-inner"
                      value={peerIdInput}
